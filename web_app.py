...
def init_files():
    files = {
        "insightflow/telemetry.json": [],
        "logs/deployment_log.csv": pd.DataFrame({
        ...
        "logs/uptime_log.csv": pd.DataFrame({
            "timestamp": [datetime.datetime.now()],
            "status": ["UP"],
            "event": ["System started"]
        }),
        "logs/healing_log.csv": pd.DataFrame({
            "timestamp": [datetime.datetime.now()],
        ...
        "logs/issue_log.csv": pd.DataFrame({
            "timestamp": [datetime.datetime.now()],
            "failure_state": ["no_failure"],
            "reason": ["System healthy"]
        }),
        "logs/rl_log.csv": pd.DataFrame({
            "retry": [0.5],
            "restore": [0.3],
            "adjust": [0.2]
        }, index=["deployment_failure"]),
        "dataset/student_scores.csv": pd.DataFrame({
    ...
    for file_path, data in files.items():
        if not os.path.exists(file_path):
            if file_path.endswith(".json"):
                ...
                data.to_csv(file_path, index=file_path=="logs/rl_log.csv")
...
def load_all_data():
    ...
    files = {
        ...
        "issues": "logs/issue_log.csv",
        "qtable": "logs/rl_log.csv",
